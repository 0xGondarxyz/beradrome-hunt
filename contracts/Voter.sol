...
contract Voter is ReentrancyGuard, Ownable {
    ...
    uint256 internal constant DURATION = 7 days; // duration of each voting epoch
    ...
    uint256 internal index; // index of current voting epoch
    mapping(address => uint256) internal supplyIndex; // plugin => index of supply at last reward distribution
    mapping(address => uint256) public claimable; // plugin => claimable rewards
    ...
    event Voter__Deposit(address indexed plugin, address indexed gauge, address account, uint256 amount);
    event Voter__Withdraw(address indexed plugin, address indexed gauge, address account, uint256 amount);
    event Voter__NotifyReward(address indexed sender, address indexed reward, uint256 amount);
    event Voter__DistributeReward(address indexed sender, address indexed gauge, uint256 amount);
     ...
     */
    function vote(address[] calldata _plugins, uint256[] calldata _weights) external onlyNewEpoch(msg.sender) {
    ...
    function distribute(address _gauge) public nonReentrant {
        ...
        _updateFor(_gauge); // should set claimable to 0 if killed //note it does not do that if it's killed?
    ...
    function addPlugin(address _plugin) external onlyGov returns (address) {
    ...
    function addBribeReward(address _bribe, address _rewardToken) external onlyGov nonZeroAddress(_rewardToken) {
        IBribe(_bribe).addReward(_rewardToken);
        emit Voter__BribeRewardAdded(_bribe, _rewardToken);
    }

    function addGaugeReward(address _gauge, address _rewardToken) external onlyGov nonZeroAddress(_rewardToken) {
    ...
    function _reset(address account) internal {
        ...
        uint256 _pluginVoteCnt = _pluginVote.length;
        uint256 _totalWeight = 0;

        for (uint256 i = 0; i < _pluginVoteCnt; i++) {
    ...
    function _vote(address account, address[] memory _pluginVote, uint256[] memory _weights) internal {
        _reset(account);
        uint256 _pluginCnt = _pluginVote.length;
        uint256 _weight = IVTOKEN(VTOKEN).balanceOf(account);
        uint256 _totalVoteWeight = 0;
        uint256 _usedWeight = 0;

        for (uint256 i = 0; i < _pluginCnt; i++) {
            address _plugin = _pluginVote[i];
